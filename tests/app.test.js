  ...
  constructor(ownerDocument) {
    ...
    this.style = {
      setProperty: (prop, value) => {
        const current = this.attributes.style
          ? `${this.attributes.style};`
          : "";
        this.attributes.style = `${current} ${prop}: ${value}`.trim();
      },
    };
    this.classList = {
      add: (...tokens) => {
...
test("AI schedule panel controls preserve its draft and accessibility state", async () => {
  ...
  const baselineStorageSize = storedValues.size;
  ...
  assert.equal(fixture.layout.classList.contains("is-ai-schedule-open"), false);
  assert.equal(storedValues.size, baselineStorageSize);
});
